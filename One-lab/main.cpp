...
void solveTask(Student students[], int count) {
    ...
    for (int j = 0; j < count; j++) {
            generalAvgGrade += students[j].avgGrade;
    }
    generalAvgGrade /= count;


    int favoriteGroupNumber = 244101;
    cout << "Enter favorite group number: "; 
    cin >> favoriteGroupNumber;

    for (int i = 0; i < count; i++) {
        if (students[i].avgGrade > generalAvgGrade && students[i].groupNumber == favoriteGroupNumber) {
            cout << "Surname: " << students[i].surname << endl;
